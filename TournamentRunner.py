...
class TournamentRunner:
    ...
    def startNewTournament(agentClass: type[IAgent], populationSize: int, generationCount: int,
        ...
        # set up logging
        log_directory = "Tournaments"
        if not os.path.exists(log_directory):
            os.makedirs(log_directory)
        logFileName = os.path.join("Tournaments", f"{getCurrentTimestamp()}_{agentClass.__name__}.txt")

        # set other variables that is used in the tournament
        ...
        # this is used to decrease the mutation rate each generation so the last generations mutation rate is 0.1
...
if __name__ == "__main__":
    ...
    TournamentRunner.startNewTournament(
        ...
        freeGenerationCount=5,  # generations to skip before save top x% and starting to decrease the mutation rate.
